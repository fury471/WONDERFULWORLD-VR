...
namespace BlobShadows
{
    public class BlobShadowsRendererPass : ScriptableRenderPass
    {
        ...
        private RTHandle _shadowMapHandle;
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            using (var _ = new ProfilingScope(cmd, ProfilingSampler))
            {
                ...
                cmd.SetGlobalTexture(ShadowMapName, _shadowMapHandle);
                cmd.SetGlobalVector(
                    ShadowMapParamsId,
        ...
        private void CullShadowCasters(Vector3 shadowFrustumCenter)
        {
            ...
            for (var index = 0; index < BlobShadows.ShadowCasters.Count; index++)
            {
                ...
                var shadowBounds = new Bounds(
                    new Vector3(x, shadowFrustumCenter.y, y),
                    new Vector3(halfSizeX, _shadowFrustumAabb.size.y, halfSizeY)
                );

                if (!GeometryUtility.TestPlanesAABB(_cameraFrustumPlanes, shadowBounds))
                    continue;
        ...
        private void DrawQuads(CommandBuffer cmd, Material material, int count)
        {
            ...
            if (material.enableInstancing && SystemInfo.supportsInstancing)
                ...
                for (var i = 0; i < count; i++)
                    cmd.DrawMesh(_quadMesh, _matrices[i], material, subMeshIndex, shaderPass);
        }
        ...
        private void RecalculateBounds(Camera camera)
        {
            ...
            _shadowFrustumAabb = new Bounds(
                (max + min) * 0.5f,
                max - min + Vector3.one * Settings.ShadowFrustumPadding
            );
